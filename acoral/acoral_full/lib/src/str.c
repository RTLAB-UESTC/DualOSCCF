 ...
 */
acoral_char * acoral_str_chr(const acoral_char * s, acoral_32 c)
{
	for(; *s != (acoral_char) c; ++s)
		if (*s == '\0')
			return NULL;
	return (acoral_char *) s;
}
 ...
 * @c: The character to search for
 */
acoral_char * acoral_str_rchr(const acoral_char * s, acoral_32 c)
{
       const acoral_char *p = s + acoral_str_len(s);
       do {
           if (*p == (acoral_char)c)
               return (acoral_char *)p;
       } while (--p >= s);
       return NULL;
}
 ...
 * @c: The character to search for
 */
acoral_char *acoral_str_nchr(const acoral_char *s, acoral_u32 count, acoral_32 c)
{
	for (; count-- && *s != '\0'; ++s)
		if (*s == (acoral_char) c)
			return (acoral_char *) s;
