...
)
{
	...
	if (!pWin->pCaretInfo) {
		...
		//printf("%d,%d\n",nWidth,nHeight);
		pWin->pCaretInfo->nBytesNr = nWidth * nHeight * _lGUI_iBytesDataType;
...
)
{
	...
	GetBoxFromScreen(sx,sy,pCaretInfo->nEffWidth,pCaretInfo->nEffHeight,
...
)
{
	...
	if (pWin->pCaretInfo->fBlink) {
		if (pWin->pCaretInfo->fShow) {
			...
			pWin->pCaretInfo->nBytesNr = newWidth*newHeight*_lGUI_iBytesDataType;
