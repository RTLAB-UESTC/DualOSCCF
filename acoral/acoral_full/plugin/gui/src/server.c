...
PDeskIconClass pAppIconClass;
BOOL GetDeskIconInfo(){
	pDeskIconInfo = (PDeskIconInfo)malloc(sizeof(DeskIconInfo));
	if(!pDeskIconInfo){
		return false;
	}
	pAppIconClass = (PDeskIconClass)malloc(sizeof(DeskIconClass));
	if(!pAppIconClass){
		return false;
	}
	pDeskIconInfo->pHead=pAppIconClass;	
	pDeskIconInfo->pTail=pAppIconClass;	
	pDeskIconInfo->iTotal=1;	

	pAppIconClass->id=1003;
	strcpy(pAppIconClass->lpszClassName,"Ó¦ÓÃ");
	strcpy(pAppIconClass->lpszIconName,"/tlgui/pics/sys/tab_app.bmp");
	pAppIconClass->iTotalIcon=0;
	pAppIconClass->pNext=NULL;
	pAppIconClass->pHead=NULL;
	pAppIconClass->pTail=NULL;
	NoteInit();
	ArtInit();
	return true;
}
