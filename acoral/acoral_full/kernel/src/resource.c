 ...
 *       count--要释放的pool的个数
 *================================*/
void acoral_release_pool(acoral_pool_ctrl_t *pool_ctrl,acoral_u32 count){
	acoral_pool_t *pool;
	acoral_list_t *list,*head;
	acoral_u32 i;
	if(count==0)
	  	count=pool_ctrl->max_pools-1;
	head=pool_ctrl->free_pools;
	if(acoral_list_empty(head))
	  	return;
        for(list=pool_ctrl->free_pools->next,i=0;list!=head&&i<count;list=list->next,i++){
...
int acoral_assert_res(acoral_res_t *res,acoral_8 *assert){
