...
void v_free(void * p){
	...
	mem_ctrl.free_num++;
#endif
	while(acoral_mutex_pend(&mem_ctrl.mutex, 0)!=0)
		acoral_suspend_self();
#ifdef CFG_TEST_MEM2
	acoral_print("...................\n");
...
void v_mem_init(){
	mem_ctrl.down_p=(acoral_8 *)acoral_malloc(MEM_SIZE);
	...
	mem_ctrl.top_p=mem_ctrl.down_p+MEM_SIZE;
	mem_ctrl.freep_p=(acoral_u32 *)mem_ctrl.down_p;
	BLOCK_SET_FREE(mem_ctrl.freep_p,MEM_SIZE);
#ifdef CFG_TEST
